...
// Основные проверки существования продукта
...
// Дополнительные проверки для цен
$has_regular_price = !empty($regular_price) && is_numeric($regular_price) && $regular_price > 0;
$has_sale_price = !empty($sale_price) && is_numeric($sale_price) && $sale_price > 0;
$is_valid_discount = false;
$discount = 0;

// Проверяем возможность расчета скидки
if ($is_on_sale && $has_regular_price && $has_sale_price) {
...
<div class="card scroll-animate" data-product-id="<?php echo esc_attr($product_id); ?>">
    <?php if ($is_valid_discount) : ?>
        <span class="card__discount">-<?php echo esc_html($discount); ?>%</span>
    <?php endif; ?>
    
    <span class="card__favourite <?php echo is_user_favorite($product_id) ? 'active' : ''; ?>" 
